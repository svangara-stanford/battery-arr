...
def validate_processed_tables(
    ...
    *,
    ...
    if cycle_summary["cycle_index"].isna().any():
        raise ValueError("cycle_summary contains missing cycle_index values")
    if (pd.to_numeric(cycle_summary["cycle_index"], errors="coerce") < 0).any():
        raise ValueError("cycle_index must be non-negative or one-indexed")
...
def qc_summary(metadata: pd.DataFrame, cycle_summary: pd.DataFrame) -> dict[str, object]:
